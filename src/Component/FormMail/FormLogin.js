...
function FormLogin() {
  ...
  const loginAdmin = async e => {
    ...
    try {
      ...
      if (res.code === "C000") {
        console.log(res.formMailAdmin.rName);
        dispatch(
          loginUser({
            userId: res.formMailAdmin.userId,
            userName: res.formMailAdmin.rName,
            phone: res.formMailAdmin.phone,
            admin: res.formMailAdmin.admin,
            role: res.formMailAdmin.role,
            lastLogin: { day: day, hour: hour, min: min },
          })
        );
      } else {
